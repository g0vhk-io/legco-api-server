...
from legco.models import Question, Individual
...
year_ranges = [(y, y + 1) for y in [12, 13, 14, 15, 16, 17]]
...
class Command(BaseCommand):
    ...
    def crawl(self, link, individual, question_type_text, title, date):
        md5 = hashlib.md5()
        ...
        encoding = w3lib.encoding.html_body_declared_encoding(detail_r.text)
        detail_r.encoding = encoding
        ...
        if reply_start == -1:
            reply_start = press_release.rfind(u'主席：')
        ...
        for y1, y2 in year_ranges:
            ...
            for i in range(0, len(dates)):
                ...
                for row in table.xpath(".//tr")[1:]:
                    ...
                    try:
                      link = cells[3].xpath(".//a")[0].attrib['href'].strip()
                      self.crawl(link, individual, question_type_text, title, date)
                    except IndexError:
                      print("" + title + "has no reply.")
                      continue
